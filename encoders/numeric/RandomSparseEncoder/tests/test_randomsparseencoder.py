...
def get_encoder(metas, train_data, target_output_dim):
    ...
    model_path = os.path.join(tmpdir, 'random_sparse.model')

    model = SparseRandomProjection(n_components=target_output_dim, random_state=42)
    model.fit(train_data)
    pickle.dump(model, open(model_path, 'wb'))

    return RandomSparseEncoder(model_path=model_path)
