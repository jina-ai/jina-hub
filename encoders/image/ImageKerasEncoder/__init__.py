...
class ImageKerasEncoder(BaseTFEncoder):
    """
    :class:`ImageKerasEncoder` encodes data from a ndarray, potentially BatchSize x (Channel x Height x Width) into a
        ndarray of BatchSize * d.

    The :class:`ImageKerasEncoder` wraps the models from `tensorflow.keras.applications`. <https://keras.io/applications/>`_.
