...
import numpy as np
from jina.executors.indexers.query.vector import NumpyQueryIndexer


class AnnoyQueryIndexer(NumpyQueryIndexer):
    """Annoy-powered query vector indexer
    ...
    def __init__(self, metric: str = 'euclidean', n_trees: int = 10, search_k: int = -1, *args, **kwargs):
        """
        Initialize an AnnoyQueryIndexer
        ...
        """
        # name is not needed
        # as query indexers should not be serialized
        # and are stateLESS
        super().__init__(*args, compress_level=0, index_filename=None, **kwargs)
        self.metric = metric
    ...
    def query(self, vectors: 'np.ndarray', top_k: int, *args, **kwargs) -> Tuple['np.ndarray', 'np.ndarray']:
        ...
        :param vectors: numpy array containing vectors to search for
        :param top_k: upper limit of responses for each search vector
        """
        all_idx = []
        all_dist = []
        for v in vectors:
            idx, dist = self.query_handler.get_nns_by_vector(v, top_k, self.search_k, include_distances=True)
