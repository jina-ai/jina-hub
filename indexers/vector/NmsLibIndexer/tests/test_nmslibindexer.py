...
def test_nmslib_indexer_known():
    vectors = np.array([[1, 1, 1],
                        [10, 10, 10],
                        [100, 100, 100],
                        [1000, 1000, 1000]], dtype=np.float32)
    keys = np.array([4, 5, 6, 7]).reshape(-1, 1)
    with NmsLibIndexer(index_filename='nmslib.test.gz') as indexer:
...
def test_nmslib_indexer_known_big():
    ...
    with NmsLibIndexer(index_filename='nmslib.test.gz', num_threads=4) as indexer:
        indexer.add(keys, vectors)
        indexer.save()
        assert os.path.exists(indexer.index_abspath)
        index_abspath = indexer.index_abspath
