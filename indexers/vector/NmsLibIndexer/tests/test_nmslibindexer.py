...
def test_nmslib_indexer(tmpdir):
...
def test_nmslib_wrap_indexer(tmpdir):
    os.environ['TEST_WORKSPACE'] = str(tmpdir)
    metas = get_default_metas()
    metas['workspace'] = os.environ['TEST_WORKSPACE']
...
def test_nmslib_indexer_known(tmpdir):
    os.environ['TEST_WORKSPACE'] = str(tmpdir)
    metas = get_default_metas()
    metas['workspace'] = os.environ['TEST_WORKSPACE']
...
def test_nmslib_indexer_known_big(tmpdir):
    ...
    with NmsLibIndexer(index_filename='nmslib.test.gz', num_threads=4, metas=metas) as indexer:
        indexer.add(keys, vectors)
        indexer.save()
        assert os.path.exists(indexer.index_abspath)
        save_abspath = indexer.save_abspath
