...
def test_faiss_indexer(tmpdir):
    os.environ['TEST_WORKSPACE'] = str(tmpdir)
    metas = get_default_metas()
    metas['workspace'] = os.environ['TEST_WORKSPACE']
    train_filepath = os.path.join(tmpdir, 'train.tgz')
    train_data = np.array(np.random.random([1024, 10]), dtype=np.float32)
    with gzip.open(train_filepath, 'wb', compresslevel=1) as f:
        f.write(train_data.tobytes())

    with FaissIndexer(index_filename='faiss.test.gz', index_key='IVF10,PQ2', train_filepath=train_filepath, metas=metas) as indexer:
...
def test_faiss_indexer_known(tmpdir):
    os.environ['TEST_WORKSPACE'] = str(tmpdir)
    metas = get_default_metas()
    metas['workspace'] = os.environ['TEST_WORKSPACE']
    ...
    with FaissIndexer(index_filename='faiss.test.gz', index_key='Flat', train_filepath=train_filepath, metas=metas) as indexer:
...
def test_faiss_indexer_known_big(tmpdir):
