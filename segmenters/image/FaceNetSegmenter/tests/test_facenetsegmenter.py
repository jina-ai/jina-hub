...
@pytest.mark.parametrize(
    "filenames_with_labels",
    [
        [("one_face.jpg", 1), (np.ones((128, 512, 3)), 0), ("three_faces.jpg", 3)],
    ],
)
def test_segment_mixed_batch(segmenter_multiface, filenames_with_labels):
    images = []
    for labelled_file, _ in filenames_with_labels:
        if type(labelled_file) == str:
            labelled_file = np.array(Image.open(os.path.join(SEGMENTER_DIR, "imgs", labelled_file)))

        images.append(cv2.resize(labelled_file, dsize=(480, 320),))
